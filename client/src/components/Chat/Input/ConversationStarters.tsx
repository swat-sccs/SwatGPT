...
const ConversationStarters = () => {
  ...
  return (
    <div className="mx-auto mb-8 mt-2 flex w-full max-w-2xl flex-wrap items-center justify-center gap-2 px-4">
      {conversation_starters
        ...
        .map((text: string, index: number) => (
          <button
            ...
            className="flex cursor-pointer items-center rounded-full border border-border-medium bg-surface-secondary px-4 py-2 text-sm text-text-secondary shadow-sm transition-colors duration-200 fade-in hover:border-border-heavy hover:bg-surface-tertiary hover:text-text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring-primary"
          >
            <span className="max-w-[20rem] truncate">{text}</span>
          </button>
        ))}
    </div>
  );
